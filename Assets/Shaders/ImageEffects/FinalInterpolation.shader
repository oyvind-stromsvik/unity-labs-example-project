...
float4 frag(v2f i) : COLOR
{
	...
	half2 unwrapped = ScreenUVToEpipolarUV(uv);
		...
		float4 positionsSample = tex2D(_SamplePositions, uv);
		c *= _ShowSamplesBackgroundFade;
		float isRaymarchSample = any(positionsSample.rgb);
		return float4(lerp(c, positionsSample, isRaymarchSample).rgb, _ShowSamplesBackgroundFade*(1 - isRaymarchSample));
	#else
