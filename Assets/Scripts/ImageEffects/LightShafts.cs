...
public partial class LightShafts : MonoBehaviour
{
	public void Start()
	{
	    ...
	    {
            Debug.LogWarningFormat("Disabling LightShafts effect on {0} because the hardware does not support it.", name);
	        enabled = false;
	        return;
	    }
	...
	void UpdateShadowmap()
	{
	    ...
	    UnityEngine.Profiling.Profiler.BeginSample("UpdateShadowmap");
		...
		if (m_ShadowmapCamera == null)
		{
			...
			m_ShadowmapCamera.useOcclusionCulling = false;
		}
    ...
    private RenderBuffer[] _coordBuffers = new RenderBuffer[2];

	void RenderCoords(int width, int height, Vector4 lightPos)
	{
	    UnityEngine.Profiling.Profiler.BeginSample("RenderCoords");

		SetFrustumRays(m_CoordMaterial);

	    _coordBuffers[0] = m_CoordEpi.colorBuffer;
	    _coordBuffers[1] = m_DepthEpi.colorBuffer;
        Graphics.SetRenderTarget(_coordBuffers, m_DepthEpi.depthBuffer);
	...
	{
	    UnityEngine.Profiling.Profiler.BeginSample("RenderInterpolationTexture");
	...
	{
	    UnityEngine.Profiling.Profiler.BeginSample("InterpolateAlongRays");
	...
	{
	    UnityEngine.Profiling.Profiler.BeginSample("RenderSamplePositions");
	...
	{
	    UnityEngine.Profiling.Profiler.BeginSample("Raymarch");
	...
	public void OnRenderObject ()
	{
	    ...
	    UnityEngine.Profiling.Profiler.BeginSample("FinalInterpolation");
