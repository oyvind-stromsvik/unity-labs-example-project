...
public partial class LightShafts : MonoBehaviour
{
	...
	bool IsVisible()
	{
	    if (m_VisibilityProxy)
	        return m_VisibilityProxy.WasVisibleThisFrame();
	...
	bool IntersectsNearPlane()
	{
		...
		Transform t = transform;
        for (int i = 0; i < m_SpotMeshVertices.Length; i++)
		{
            float z = m_CurrentCamera.WorldToViewportPoint(t.TransformPoint(m_SpotMeshVertices[i])).z;
	...
	void GetFrustumRays(out Matrix4x4 frustumRays, out Vector3 cameraPosLocal)
	{
		...
		for (int i = 0; i < 4; i++)
		{
            Vector2 uv = new Vector2(((i+1)&0x02) >> 1, i >> 1); // Should produce (0,0), (1,0), (1,1), (0,1)
			Vector3 ray = m_CurrentCamera.ViewportToWorldPoint(new Vector3(uv.x, uv.y, far)) - cameraPos;
